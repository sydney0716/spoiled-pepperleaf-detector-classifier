#!/usr/bin/env python3
"""
Run YOLO detection and ResNet classification on a single image.

For one input image, this script:
  1. Detects pepper leaves using the trained YOLO checkpoint.
  2. Crops each detection and classifies it as normal/spoiled with the trained
     ResNet classifier.
  3. Writes an annotated image showing the bounding boxes plus classification
     results. Optionally saves the cropped leaves.

Default checkpoints:
  - Detection:   weights/trained/best_yolov8n.pt
  - Classification: weights/trained/leaf_classifier_resnet18.pth
"""
...
DEFAULT_DET_WEIGHTS = PROJECT_ROOT / "weights/trained/best_yolov8n.pt"
DEFAULT_CLS_WEIGHTS = PROJECT_ROOT / "weights/trained/leaf_classifier_resnet18.pth"
DEFAULT_OUTPUT = PROJECT_ROOT / "results/pipeline"
...
def build_classifier(num_classes: int = 2) -> nn.Module:
    """Mirror the ResNet18 classifier head used during training."""
...
def clamp_box(x1: float, y1: float, x2: float, y2: float, width: int, height: int) -> Tuple[int, int, int, int]:
    """Ensure box coordinates stay within image bounds."""
    return (
        max(0, int(x1)),
        max(0, int(y1)),
        min(width, int(x2)),
